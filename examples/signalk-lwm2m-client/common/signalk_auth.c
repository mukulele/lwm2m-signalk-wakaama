...
bool signalk_auth_process_response(const char *json_response) {
    ...
    if (!json_object_object_get_ex(root, "requestId", &request_id_obj)) {
        json_object_put(root);
        return false; // Not an auth response
    }
    
    const char *response_request_id = json_object_get_string(request_id_obj);
    ...
    int result_code = json_object_get_int(result_obj);
    if (result_code != 200) {
    ...
    if (!json_object_object_get_ex(login_obj, "token", &token_obj)) {
        json_object_put(root);
        return false;
    }
    ...
    if (json_object_object_get_ex(login_obj, "timeToLive", &ttl_obj)) {
